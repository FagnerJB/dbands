...
class Spotify
{
   ...
   {
      return "$this->auth_url/authorize?" . http_build_query(array(
   ...
   public function save_token($code)
   {
      ...
      $token_response = wp_remote_post("$this->auth_url/api/token", array(
         'headers' => array(
      ...
      $me_response = wp_remote_get("$this->base_url/me", array(
         'headers' => array(
   ...
   private function get_user($user_hash)
   {
      if (false !== strpos($user_hash, $this->cookie_prefix)) {
   ...
   public function search($term, $type)
   {
      ...
      if ($this->token) {
         $search_response = wp_remote_get("$this->base_url/search?" . http_build_query(array(
            'q'     => $term,
            'type'  => $type,
            'limit' => 1
         )), array(
         ...
         if (!empty($search_body["{$type}s"]['items'])) {
            $searched_ID = $search_body["{$type}s"]['items'][0]['id'];
            return $searched_ID;
         }
      }
   }
   ...
   public function fetch_tracks($id)
   {
      ...
      $tracks_return = [];
      if (is_array($tracks_fetched) and count($tracks_fetched['tracks']) > 0) {
