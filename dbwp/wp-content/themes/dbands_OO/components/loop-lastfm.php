   ...
   <ul class="grid auto-rows-auto grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1 sm:gap-3 flex-wrap">
      ...
      ?>
         <li class="flex flex-col font-medium aspect-square xl:aspect-video overflow-hidden">
            <a class="group relative flex flex-col gap-1 justify-end py-1 sm:py-2 px-2 sm:px-3 size-full bg-neutral-800 text-zinc-200 hover:bg-neutral-700 focus:bg-neutral-700" href="<?php echo $artist['url'] ?>" target="_blank">
               <div class="text-lg sm:text-xl line-clamp-2"><?php echo $artist['name'] ?></div>
            ...
            <a class="flex justify-between items-center py-1 sm:py-2 px-3 bg-amber-400 text-zinc-800 hover:bg-yellow-300 focus:bg-yellow-300" href="<?php echo Utils::get_search_link($artist['name'], 'artista') ?>" title="Buscar similares de <?php echo $artist['name'] ?>" rel="nofollow">
