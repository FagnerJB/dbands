         ...
         <ul class="flex flex-wrap gap-1 sm:gap-2 text-xs sm:text-sm">
            ...
            if (!empty($lastfm['tags'])) {
               foreach ($lastfm['tags'] as $item) {
                  ...
                  ?>
                  <li>
                     <a class="btn !gap-2" href="<?php echo $tag_url; ?>" rel="nofollow external">
...
?>
            <li>
               <a class="btn !gap-2" href="<?php echo $lastfm['url']; ?>" target="_blank" rel="nofollow external">
...
?>
      ...
      foreach ($lastfm['items'] as $artist) {
         ...
         <li class="flex flex-col font-medium aspect-square xl:aspect-video overflow-hidden">
            <a class="group relative flex flex-col gap-1 justify-end py-1 sm:py-2 px-2 sm:px-3 size-full bg-neutral-800 text-zinc-200 hover:bg-neutral-700 focus-visible:bg-neutral-700" href="<?php echo $artist['url']; ?>" target="_blank" rel="nofollow external">
            ...
            <a class="flex justify-between items-center py-1 sm:py-2 px-3 bg-red-700 text-zinc-200 hover:bg-red-600" href="<?php echo Utils::get_search_link($artist['name'], 'videos'); ?>" title="Buscar vídeos de <?php echo $artist['name']; ?>" rel="nofollow external">
               Buscar vídeos <i class="fa-brands fa-youtube"></i>
            </a>
            <a class="flex justify-between items-center py-1 sm:py-2 px-3 bg-amber-400 text-zinc-800 hover:bg-yellow-300 focus-visible:bg-yellow-300" href="<?php echo Utils::get_search_link($artist['name'], 'artista'); ?>" title="Buscar similares de <?php echo $artist['name']; ?>" rel="nofollow external">
